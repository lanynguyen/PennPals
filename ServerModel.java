...
public final class ServerModel {
    ...
    private TreeMap<Integer, String> allClients;
    /**
    ...
    public int getUserId(String nickname) {
        for (Map.Entry<Integer, String> entry : allClients.entrySet()) {
    ...
    public Collection<String> getRegisteredUsers() {
        Collection<String> registeredUsers = new TreeSet<>(allClients.values());
    ...
    public Collection<String> getChannels() {
        Collection<String> channels = new TreeSet<>(allChannels.keySet());
        return channels;
    }
    ...
    public Collection<String> getUsersInChannel(String channelName) {
        Collection<String> usersInChannel = new TreeSet<>();
        if (allChannels.containsKey(channelName)) {
            usersInChannel.addAll(allChannels.get(channelName).getClients());
        }
    ...
    public String getOwner(String channelName) {
        if (allChannels.containsKey(channelName)) {
            return allChannels.get(channelName).getOwner();
        }
        return null;
    }
    ...
    public Broadcast registerUser(int userId) {
        ...
        allClients.put(userId, nickname);
    ...
    public Broadcast deregisterUser(int userId) {
        TreeMap<String, Channel> temp = new TreeMap<>();
        temp.putAll(allChannels);

        String nickname = allClients.get(userId);
        Collection<String> broadcastList = new TreeSet<>();

        for (Map.Entry<String, Channel> entry : temp.entrySet()) {
            Channel currChannel = entry.getValue();
            if (currChannel.getClients().contains(nickname)) {
                currChannel.removeClient(nickname);
                broadcastList.addAll(currChannel.getClients());
                if (currChannel.getOwner().equals(nickname)) {
                    allChannels.remove(entry.getKey());
                }
            }
        }
        allClients.remove(userId);
        return Broadcast.disconnected(nickname, broadcastList);
    }
    ...
    public Broadcast changeNickname(NicknameCommand nickCommand) {
        ...
        } else if (isValidName(newName) && allClients.containsValue(newName)) {
            return Broadcast.error(nickCommand, ServerResponse.NAME_ALREADY_IN_USE);
        }
        Collection<String> broadcastList = new TreeSet<>();
        for (Channel currChannel : allChannels.values()) {
            if (currChannel.getClients().contains(oldName)) {
                if (currChannel.getOwner().equals(oldName)) {
                    currChannel.setOwnerName(newName);
                }
                currChannel.removeClient(oldName);
                currChannel.addClient(newName);
                broadcastList.addAll(currChannel.getClients());
            }
        }
        allClients.replace(id, oldName, newName);
        return Broadcast.okay(nickCommand, broadcastList);
    }
    ...
    public Broadcast createChannel(CreateCommand createCommand) {
        ...
        Channel newChannel = new Channel(ownerName, channelName, privacy);
        allChannels.put(channelName, newChannel);

        Collection<String> broadcastList = new TreeSet<>();
        broadcastList.add(ownerName);
        return Broadcast.okay(createCommand, broadcastList);
    }
    ...
    public Broadcast joinChannel(JoinCommand joinCommand) {
        String nickname = joinCommand.getSender();
        String channelName = joinCommand.getChannel();
        Channel channel = allChannels.get(channelName);
        if (!allChannels.containsKey(channelName)) {
            return Broadcast.error(joinCommand, ServerResponse.NO_SUCH_CHANNEL);
        } else if (allChannels.containsKey(channelName) && channel.getPrivacy()) {
            return Broadcast.error(joinCommand, ServerResponse.JOIN_PRIVATE_CHANNEL);
        }
        channel.addClient(nickname);
        Collection<String> broadcastList = getUsersInChannel(channelName);
        return Broadcast.names(joinCommand, broadcastList, channel.getOwner());
    }
    ...
    public Broadcast leaveChannel(LeaveCommand leaveCommand) {
        String nickname = leaveCommand.getSender();
        ...
        Collection<String> broadcastList = getUsersInChannel(channelName);
        if (channel.getOwner().equals(nickname)) {
            allChannels.remove(channelName);
        }
        channel.removeClient(nickname);
        return Broadcast.okay(leaveCommand, broadcastList);
    }
    ...
    public Broadcast inviteUser(InviteCommand inviteCommand) {
        ...
        if (!channel.getOwner().equals(inviter)) {
            return Broadcast.error(inviteCommand, ServerResponse.USER_NOT_OWNER);
        }
        channel.addClient(invitee);
        Collection<String> broadcastList = getUsersInChannel(channelName);
        return Broadcast.names(inviteCommand, broadcastList, channel.getOwner());
    }
    ...
    public Broadcast kickUser(KickCommand kickCommand) {
        ...
        if (!channel.getClients().contains(kicked)) {
        ...
        if (channel.getOwner().equals(kicked)) {
            allChannels.remove(channelName);
        }
        channel.removeClient(kicked);
        return Broadcast.okay(kickCommand, broadcastList);
    }
